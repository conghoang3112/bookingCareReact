...
class UserRedux extends Component {
    ...
    checkValidateInput = () => {
        let isValid = true;
        let arrcCheck = ['email', 'password', 'firstName', 'lastName', 'phoneNumber', 'address'];
        for (let i = 0; i < arrcCheck.length; i++) {
            if (!this.state[arrcCheck[i]]) {
                isValid = false;
                alert('This input is required: ' + arrcCheck[i])
                break;
            }
        }
        return isValid
    ...
    handleEditUserFromParent = (user) => {
        console.log('check handee edit user from parent', user);
        this.setState({
            email: user.email,
